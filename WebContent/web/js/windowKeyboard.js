  ...
  try {
    if(obj.tagName == 'A') {
      ...
      obj.className.indexOf('dojoValidateRange') != -1)) {
        ...
        obj.className = obj.className.replace('Radio_NOT_Focused', 'Radio_Focused');
        var obj_tmp = obj;
        try {
          for(;;) {
  ...
  try {
    if(obj.tagName == 'A') {
      ...
      if (obj.getAttribute('type')=='checkbox') {
        ...
        obj.className = obj.className.replace('Radio_Focused','Radio_NOT_Focused');
        var obj_tmp = obj;
        try {
          for(;;) {
...
function windowUpKey() {
  if (isGridFocused) {
    dijit.byId('grid').goToPreviousRowKey();
  } else if (isGenericTreeFocused) {
    gt_goToPreviousNode();
  }
}
...
function windowDownKey() {
  if (isGridFocused) {
    dijit.byId('grid').goToNextRowKey();
  } else if (isGenericTreeFocused) {
    gt_goToNextNode();
  }
}
