...
public class BOMReturn  extends HttpSecureAppServlet {
      ...
      if (vars.commandIn("CANCEL")){
       ...
       if (vars.commandIn("DONE")){
         ...
         if (!strConsumptionid.equals("")) {
           ...
           vars.setSessionValue("PDCSTATUSTEXT",Utility.messageBD(this, "pdc_MaterialReturnSucessful",vars.getLanguage())+msgtext);
         ...
         try {
            ...
            // Check if Value Updates a line or deletes a line
             ...
             if (sameline.equals("")) {
               ...
               BOMManagementData.insertMaterialLine(conn,this,strConsumptionLineId, vars.getClient(), vars.getOrg(), 
                   ...
                   strpdcWorkstepID,strpdcAssemblyID);
               if (! strSnrID.isEmpty() || ! strBatchID.isEmpty()){
                 BOMManagementData.insertSerialLine(conn,this, vars.getClient(), vars.getOrg(),
       ...
       EditableGrid lowergrid = new EditableGrid("PdcMaterialReturnLowerGrid", vars, this);  // Load lower grid structure from AD (use AD name)
       lowerGridData = PdcMaterialReturnData.selectlower(this, vars.getLanguage(),strConsumptionid);
       ...
       String strSkeleton = ConfigureFrameWindow.doConfigure(this,vars,"inpmProductId",null, "BOM Return",strToolbar,"NONE",tabs);
