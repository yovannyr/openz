  ...
  */
      ...
      END IF; -- isempty(NEW.m_product_id)
       IF (TG_OP = 'UPDATE') THEN
            ...
            if old.istaskcancelled!=new.istaskcancelled or old.iscomplete!=new.iscomplete then
                if (select count(*) from c_project where c_project_id=new.c_project_id and projectstatus in ('CL','OC'))=1 then
...
*/
      IF  new.isactive='Y' THEN
        ...
        LOOP
           ...
           select cv.materialcost into v_percent from ma_indirect_cost_value cv, ma_indirect_cost c where cv.ma_indirect_cost_id=c.ma_indirect_cost_id
                       and c.ma_indirect_cost_id=v_cur.ma_indirect_cost_id and cv.datefrom<=now() and cv.cost_uom='P' and cv.isactive='Y'
                       and c.isactive='Y' and c.cost_type='M' and cv.cost_uom='P' LIMIT 1;
           if coalesce(v_percent,0)>0 then
              v_bomamt:=round((coalesce(v_hramt,0)+coalesce(v_maamt,0)+coalesce(v_expenseamt,0)+coalesce(v_extserviceamt,0))*v_percent/100,2);
           end if;
        END LOOP;
        -- Sum on Task so far
...
CREATE or replace FUNCTION zspm_updateprojectstatus(p_isexplicit character varying,v_dummy varchar) RETURNS void
AS $_$
DECLARE
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
    ...
    if  (select requested from c_project_processstatus limit 1)='Y'  then -- Process ist requested
        ...
        loop
            ...
            select sum(case when ad_get_docbasetype(c_invoice.c_doctype_id)='ARC' then 
                                    case when c_invoice.isgrossinvoice='Y' and t.rate>0 then round(linegrossamt-linegrossamt/(1+100/t.rate),2)*-1 else case when c_invoice.isgrossinvoice='Y' and t.rate=0 then coalesce(linegrossamt,0)*-1 else coalesce(linenetamt,0)*-1 end end else 
                                    case when c_invoice.isgrossinvoice='Y' and t.rate>0 then round(linegrossamt-linegrossamt/(1+100/t.rate),2) else case when c_invoice.isgrossinvoice='Y' and t.rate=0 then coalesce(linegrossamt,0) else coalesce(linenetamt,0) end end end)  
            ...
            loop
                ...
                select cv.materialcost into v_percent from ma_indirect_cost_value cv, ma_indirect_cost c where cv.ma_indirect_cost_id=c.ma_indirect_cost_id
                       and c.ma_indirect_cost_id=v_cur2.ma_indirect_cost_id and cv.datefrom<=now() and cv.cost_uom='P' and cv.isactive='Y'
                       and c.isactive='Y' and c.cost_type='M' and cv.cost_uom='P' LIMIT 1;
                if coalesce(v_percent,0)>0 then
                    v_materialcost:=round((v_workcost+v_machinecost+v_extservicecost+v_glcost+v_glextservice+v_invoicecost)*v_percent/100,2);
                end if;
            end loop;
  ...
  ELSE
    ...
    --  RAISE NOTICE 'TG_NAME=''%'', new.ad_user_id=%', TG_NAME, new.ad_user_id; 
     ...
     if v_salary_id is not null then
        ...
        select p_cost, p_specialtime1, p_specialtime2 into v_cost, v_sp1,v_sp2 from zsco_get_salary_cost(v_salary_id,new.workdate,'H',v_c_projecttask.ad_org_id);
...
BEGIN
  ...
  -- Projekttyp bestimmen: pruefen, ob Produktionsprojekt moeglich
    ...
    for v_cur in (select
                ...
                case when v_projectcategory='P' then scheddeliverydate else null end as DATEFINISH,
                salesrep_id,M_WAREHOUSE_ID, v_projectcategory as PROJECTCATEGORY, 'OP' as PROJECTSTATUS, 
                case when v_projectcategory='P' then  trunc(now()) else scheddeliverydate end as STARTDATE
...
BEGIN
  ...
  IF (TG_OP <> 'DELETE') THEN
     select p_cost, p_specialtime1, p_specialtime2 into v_cost, v_sp1, v_sp2 from zsco_get_salary_cost(new.c_salary_category_id,now(),new.Costuom,new.ad_org_id);
    ...
    -- QTY Correction END
    if new.isactive='Y' then
    raise notice '%',v_cost||'Q'||new.quantity||'h'||v_hours;
...
--    RAISE NOTICE '%: zspm_projecttaskbom_id=%, v_oneqty=%, NEW.qty=%, v_prec=%', TG_NAME, v_cur.zspm_projecttaskbom_id, v_oneqty, NEW.qty, v_prec;
  ...
  if coalesce(new.materialcostplan,0)!=coalesce(old.materialcostplan,0) or coalesce(new.machinecostplan,0)!=coalesce(old.machinecostplan,0) or
     ...
     coalesce(new.externalserviceplan,0)!=coalesce(old.externalserviceplan,0) or coalesce(new.indirectcostplan,0)!=coalesce(old.indirectcostplan,0)
  then
    perform zspm_updateprojectstatus(null,null);
  end if;
...
BEGIN
      ...
      for v_cur in (select p.ma_indirect_cost_id,pv.m_product_id,t.c_projecttask_id,t.istaskcancelled
                    ...
                    (select cv.ma_indirect_cost_value_id from ma_indirect_cost_value cv,ma_indirect_cost c where cv.ma_indirect_cost_id=c.ma_indirect_cost_id
                            ...
                            and datefrom<=now() and cv.cost_uom='P' and c.isactive='Y'  LIMIT 1) 
...
BEGIN
      ...
      for v_cur in (select sum(plannedamtemp) as plannedamtemp, sum(actualcostamounts) as actualcostamounts,m_product_id,'' as description
                    ...
                    (select 0 as plannedamtemp,sum(case when ad_get_docbasetype(i.c_doctype_id)='APC' then 
                             ...
                             case when i.isgrossinvoice='Y' and t.rate>0 then round(linegrossamt-linegrossamt/(1+100/t.rate),2) else case when i.isgrossinvoice='Y' and t.rate=0 then coalesce(linegrossamt,0) else coalesce(linenetamt,0) end end end)  
...
BEGIN
    ...
    IF (p_ProductionOrder_ID is not null and p_ProductionOrderValue is not null ) THEN
      ...
      LOOP
        ...
        else -- Only the Task within the same Project
...
select zsse_droptrigger('c_projecttask_check_trg','c_orderline');
