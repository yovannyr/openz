...
BEGIN
   ...
   IF TG_OP != 'DELETE' THEN 
       ...
       if (select count(*) from snr_inventoryline snr,m_inventoryline ml 
            where snr.m_inventoryline_id=ml.m_inventoryline_id 
            and ml.m_inventory_id=(select m_inventory_id from m_inventoryline where m_inventoryline_id=new.m_inventoryline_id)
            and ml.m_product_id=(select m_product_id from m_inventoryline where m_inventoryline_id=new.m_inventoryline_id)
            and coalesce(snr.serialnumber,'#')=coalesce(new.serialnumber,'-') 
            and snr_inventoryline_id!=new.snr_inventoryline_id)>0
       then
                raise exception '%', 'Serial Number double';
       end if;
       if v_return!='OK' then raise exception '%', v_return; end if;
   else
         ...
         if p_table='inventory' then
             ...
             if v_isserial='Y' or v_isbatch='Y' then
                 ...
                 LOOP
                    ...
                    select snr_masterdata_id into v_snrmdid from snr_masterdata where serialnumber=coalesce(v_snrcur.serialnumber,'not available') and m_product_id=v_cur.m_product_id;
...
BEGIN
    ...
    if issotrx='N' then
        ...
        select movementdate into v_mmdate from m_inout,m_inoutline where m_inoutline.m_inout_id=m_inout.m_inout_id and m_inoutline.m_inoutline_id=v_minoutline_id;
...
v_qty numeric;
BEGIN
    select m.movementtype,ml.c_projecttask_id,p.isserialtracking,ml.movementqty,m.updatedby,m.ad_org_id into v_type,v_task,v_serial,v_qty,v_user,v_org
    ...
    if v_type='P+' and v_serial='Y' and v_qty>0 then
