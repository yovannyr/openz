...
BEGIN
  ...
  IF(TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
    select count(*) into v_count from c_invoice i  where i.c_invoice_id=new.c_invoice_id and i.issotrx='Y';
    if v_count>0 then
              ...
              if v_draftexists>0 then
                    ...
                    INTO C_INVOICE
                      ...
                      (
                        ...
                        coalesce(Cur_Order.contractdate,Cur_Order.scheddeliverydate),Cur_Order.enddate,v_internalnote,v_scheduledtransactiondate,v_transactionreference
                      )
                      ;
                          ...
                          INTO C_INVOICELINE
                            (
                              ...
                              C_Tax_ID,  --MODIFIED BY F.IRIAZABAL
                               M_Product_Uom_ID, PriceStd,M_InOutLine_ID,quantityorder ,m_manufacturer_id,
                              m_attributesetinstance_id,isgrossprice,c_project_id ,c_projectphase_id ,c_projecttask_id ,a_asset_id,pricefluctuation 
                            )
                            VALUES
                            (
                              ...
                              v_ĺinesrecord.M_Product_Uom_ID, v_ĺinesrecord.PriceStd,v_ĺinesrecord.M_InOutLine_ID,v_qty2nduom,v_ĺinesrecord.m_manufacturer_id,
                              v_ĺinesrecord.m_attributesetinstance_id,v_isgross,v_ĺinesrecord.c_project_id ,v_ĺinesrecord.c_projectphase_id ,
                              v_ĺinesrecord.c_projecttask_id ,v_ĺinesrecord.a_asset_id,v_ĺinesrecord.pricefluctuation
                            )
                            ;
                            -- SZ END
                        END IF;--Next_O_Line
              ...
              ************************************************************************/
              ...
              INTO C_INVOICELINE
                (
                  ...
                  C_UOM_ID, C_Tax_ID, PriceStd,isgrossprice,pricefluctuation)
                VALUES
                (
                  ...
                  Cur_InvoiceLine.C_UOM_ID, Cur_InvoiceLine.C_Tax_ID, Cur_InvoiceLine.PriceStd,Cur_InvoiceLine.isgrossprice,Cur_InvoiceLine.pricefluctuation * -1)
                ;
...
---- <<FINISH_PROCESS>>
    ...
    if (select count(*) from c_invoiceline where c_invoice_id=v_Record_ID and c_project_id is not null)>0 then
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' then
    ...
    if new.processed='Y' and old.processed!='Y' and new.issotrx='N' then
      for v_cur_line in (select * from c_invoiceline where c_invoice_id=new.c_invoice_id)
      LOOP
        ...
        if v_iscalcenabled='Y' then 
            ...
            if v_dummy>0 then
                ...
                insert into m_product_po(M_PRODUCT_PO_ID,M_PRODUCT_ID, C_BPARTNER_ID, AD_CLIENT_ID, AD_ORG_ID, ISACTIVE, CREATED, CREATEDBY, UPDATED, UPDATEDBY, ISCURRENTVENDOR, C_CURRENCY_ID,PRICELIST, PRICEPO,
                                        PRICEEFFECTIVE, PRICELASTINV,c_uom_id,m_manufacturer_id)
                            values (get_uuid(),v_cur_line.m_product_id,new.C_BPARTNER_ID,new.AD_CLIENT_ID,new.AD_ORG_ID,'Y',now(),new.CREATEDBY,now(),new.UPDATEDBY,'Y',new.C_CURRENCY_ID,v_cur_line.pricelist,v_cur_line.priceactual,
                                        now(),v_cur_line.priceactual,v_uom,v_cur_line.m_manufacturer_id);
            end if;
        end if;
      END LOOP;
    end if;   
...
BEGIN
    ...
    for v_cur in (SELECT AD_Client_ID,AD_Org_ID,C_OrderLine_ID,Description,M_Product_ID,Qtyordered,
                  PriceList,PriceActual,C_UOM_ID,C_Tax_ID,Line,PriceStd,m_attributesetinstance_id,c_project_id,c_projecttask_id,c_order_id,
                  c_doctype_id, datepromised, dateordered,documentno,c_bpartner_id, pendingamt,  pendingqty,  pendingprice 
                 from c_invoice_candidate_lines_v where c_doctype_id='6C8EA6FFBB2B4ACBA0542BA4F833C499' order by c_bpartner_id,c_order_id,line)
    LOOP
        ...
        INSERT INTO C_INVOICELINE
                ...
                C_Invoice_ID, 
                Line, Description,
                M_Product_ID, QtyInvoiced, PriceList, PriceActual,  C_UOM_ID, C_Tax_ID)
            VALUES
                (get_uuid(),v_cur.C_OrderLine_ID,
                v_cur.ad_client_id,v_cur.ad_Org_ID, v_User,v_User,
                v_C_Invoice_ID,v_lines,
                v_cur.description,
                v_cur.m_product_id,v_cur.pendingqty,v_cur.pendingprice,v_cur.pendingprice,v_cur.c_UOM_ID, v_cur.c_Tax_ID);
    ENd LOOP;
    if v_lines>0 and v_C_Invoice_ID is not null then
          PERFORM C_INVOICE_POST(NULL, v_C_Invoice_ID) ; 
    end if;
