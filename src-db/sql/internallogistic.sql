...
CREATE OR REPLACE FUNCTION ils_getSerialsStringFromINOUTLine(p_inoutline_id varchar) RETURNS varchar
    LANGUAGE plpgsql
    AS $_$ DECLARE 
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
v_cur RECORD;
BEGIN
    for v_cur in (select serialnumber from snr_minoutline where m_inoutline_id=p_inoutline_id)
    LOOP
        if length(v_return)>1 then
            v_return:=v_return||', ';
        end if;
        v_return:=v_return|| v_cur.serialnumber;
    END LOOP;
    return v_return;
END; $_$;
...
CREATE OR REPLACE FUNCTION ils_getLastSerialsUpdatedPersonFromINOUTLine(p_inoutline_id varchar) RETURNS varchar
    LANGUAGE plpgsql
    AS $_$ DECLARE 
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
v_cur RECORD;
BEGIN
    for v_cur in (select snr.updatedby from snr_minoutline sio,snr_masterdata snr 
                  where sio.snr_masterdata_id=snr.snr_masterdata_id and sio.m_inoutline_id=p_inoutline_id order by snr.updated desc)
    LOOP
        return v_cur.updatedby;
    END LOOP;
    return v_return;
END; $_$;
...
SELECT                                                           
	 ...
	 m_inout.directmail AS directmail,                              
         ...
         m_inoutline.isinvoiced AS isinvoiced,                                 
         m_inoutline.ad_user_id AS ad_user_id,
         ils_getSerialsStringFromINOUTLine(m_inoutline.m_inoutline_id) as serialnumber,
         zssi_getusername(ils_getLastSerialsUpdatedPersonFromINOUTLine(m_inoutline.m_inoutline_id)) as snremployee,
