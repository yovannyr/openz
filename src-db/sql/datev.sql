                        ...
                        then
                            ...
                            if v_count!=1 and v_cur2.ad_table_id='318' then
                                ...
                                LOOP
                                    ...
                                    v_line:=v_line+1;
                                    v_desc:=replace(substr(v_cur2.description,1,30),E'\r\n','');
                                    v_satz:= ';'||v_xsource||';'||v_stramt||';'||v_korrektur||';'||v_dracct||';'||v_belegfeld1||';'||v_belegfeld2||';'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                                    if v_satz is null then
                                        raise exception '%','Datensatz ist NULL e: '||v_cur2.fact_acct_id;
                                    end if;
                                    insert into ZSDV_Datev_ExportLines (ZSDV_Datev_ExportLines_id,ZSDV_DATEV_EXPORT_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, dateacct,fact_acct_group,lineno, export_data )
...
-- Kopfsatz kopieren
  ...
  for v_cur in (select * from i_susa)
  LOOP
    select zsfi_susadatevaccounts_id,c_elementvalue_id into v_id,v_elemvalid from zsfi_susadatevaccounts where lpad(name,5,'0')=lpad(v_cur.i_01,5,'0') and zsfi_susadatev_id=p_susadatevid;
    if v_id is not null then
        -- Kontotyp bestimmen
        SELECT TRIM(accounttype) INTO v_accounttype FROM c_elementvalue WHERE c_elementvalue_id=v_elemvalid AND accounttype in ('A','L','O', 'E', 'R');
