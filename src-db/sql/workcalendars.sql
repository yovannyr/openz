...
BEGIN 
   ...
   LOOP     
    select     sum(dtotal),    sum(dsoll) into v_work,    v_target 
...
dsoll:=v_time;
for v_cur in (select workdate, to_char(workdate,'DY') as vdd,to_char(workdate,'MONTH') as vm, to_char(workdate,'YYYY') as vy, to_char(workdate,'DD') as vdn,c_project_id,  hour_from, hour_to, hours, issunday, issaturday, isholiday, dayhours, breaktime, zspm_ptaskfeedbackline_id, traveltime, specialtime, specialtime2,triggeramt, overtimehours, c_projecttask_id, nighthours, ad_user_id 
