  ...
  BEGIN --BODY
    ...
    if (p_timehorizon<10000) then
     ...
     LOOP
        ...
        v_refdate:= to_date(mrp_getsheddeliverydate4vendorProduct(null,Cur_Product.M_PRODUCT_ID,p_org_id,null,null),'dd.mm.yyyy');
        p_timehorizon:=zssi_NumofWorkdays2CaleandarDaysFromGivenDate(p_timehorizon,p_org_id,v_refdate) ;
        select v_refdate + p_timehorizon + v_daysbetweenplandateandnow into v_scheddeliverydate;
        delete from MRP_RUN_PURCHASELINE where m_product_id=v_cur.M_Product_ID and planneddate>v_scheddeliverydate and planneddate>p_planningdate;
     END LOOP;
    end if;
  END; --BODY
EXCEPTION
...
BEGIN
  BEGIN --BODY
    ...
    -- alle bisherigen Datensaetze (mrp_run_purchaseline) zu einem Produkt/Artikel verarbeiten:
        ...
        SELECT PO.C_BPARTNER_ID,   coalesce(qtystd,0),   ismultipleofminimumqty, coalesce(order_min,0), coalesce(deliverytime_promised,1) as deliverytime_promised,pricepo,c_uom_id,m_manufacturer_id -- qtytype
        INTO           v_vendor, v_qtystd, v_ismultipleofminimumqty,  v_qtymin, v_DELAYMIN, v_price  ,v_uom,v_manufacturer         -- v_qtytype
        ...
        v_daysbetweenplandateandnow:=trunc(p_planningdate)-trunc(now());
        select to_date(mrp_getsheddeliverydate4vendorProduct(v_vendor,Cur_PlanProduct.M_PRODUCT_ID,p_org_id,v_uom,v_manufacturer),'dd.mm.yyyy')  + v_daysbetweenplandateandnow 
               into v_scheddeliverydate;
        -- Spätere Lieferungen decken den aktuellen Bedarf ggf. nicht..
        select sum(qty) into  v_auxQty from MRP_RUN_PURCHASELINE where MRP_RUN_PURCHASE_ID = p_Run_ID  and M_PRODUCT_ID=Cur_PlanProduct.M_PRODUCT_ID
        ...
        if v_auxQty< v_neededQty_New then
            v_neededQty_New:=v_auxQty;
        end if;
        ...
        IF (v_neededQty_New < 0) THEN            
            ...
            if v_neededQty_New < 0 then
                ...
                select sum(qty)*-1 into v_alldemand from MRP_RUN_PURCHASELINE where inouttrxtype not in ('PP','IQ','OS','MS','SF') and MRP_RUN_PURCHASE_ID = p_Run_ID
                ...
                update mrp_run_purchaseline set neededqty = qty,pricelist=v_price,framecontractline=v_framecontract,totaldemand=v_alldemand,
                                                m_manufacturer_id=v_manufacturer,c_uom_id=v_prodUOM,m_product_uom_id=v_2nduom,
    ...
    FOR Cur_workproposal IN (
      ...
      IF (v_count > 0) THEN
        SELECT PriceList, Pricepo as PriceStd,
               M_Get_Offers_Price(v_DateDoc,Cur_workproposal.C_BPartner_ID,Cur_workproposal.M_Product_ID,coalesce(Cur_workproposal.quantityorder,Cur_workproposal.QTY), Cur_workproposal.PO_PRICELIST_ID,'N',null,'N',null,v_2nduom,Cur_workproposal.m_manufacturer_id),
      ...
      INSERT INTO C_OrderLine
        (C_ORDERLINE_ID, AD_CLIENT_ID, AD_ORG_ID, ISACTIVE,
         ...
         C_TAX_ID,quantityorder,m_product_uom_id,m_manufacturer_id,orderlineselfjoin)
     VALUES
      (v_COrderLine_ID,v_Client_ID, v_Org_ID,'Y',
       ...
       v_TaxID,Cur_workproposal.quantityorder,Cur_workproposal.m_product_uom_id,Cur_workproposal.m_manufacturer_id,
...
BEGIN
 -- bei negativen Bestellmengen bzw. Bestellmengen=0 werden Standard- bzw. Mindest-Bestellmengen nicht bruecksichtigt
  IF ( COALESCE(p_qty, 0) >= 0 ) THEN -- kein negativen Bestellmengen (bei neg. BestellMenge beachte: a) CEIL(-2.0 / 20.0)= 0; b) -2.0/20.0= -0,1, CEIL(2.0 / 20.0)=1
    ...
    and case when p_ManufacturerId is not null then mpo.m_Manufacturer_Id=p_ManufacturerId else mpo.m_Manufacturer_Id is null end;
...
BEGIN
  ...
  and case when p_ManufacturerId is not null then mpo.m_Manufacturer_Id=p_ManufacturerId else 1=1 end;
...
CREATE or replace FUNCTION mrp_getsheddeliverydate4vendorProduct(p_vendor_id varchar,p_product_id varchar,p_org_id varchar,p_uom_id varchar,p_manufacturer_id varchar)  RETURNS varchar
AS $_$
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
compliance with the License. You may obtain a copy of the License at http://www.mozilla.org/MPL/MPL-1.1.html
...
BEGIN
    if p_vendor_id is null then
        ...
        SELECT po.M_PRODUCT_PO_id
                ...
                case when p_manufacturer_id is not null then p_manufacturer_id=m_manufacturer_id else m_manufacturer_id is null end 
                ORDER BY COALESCE(po.qualityrating,0) desc  LIMIT 1;
    end if;
