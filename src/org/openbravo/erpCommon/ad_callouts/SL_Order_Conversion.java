...
public class SL_Order_Conversion extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strManufacturer = vars.getStringParameter("inpmManufacturerId");

      try {
        printPage(response, vars, strUOM, strMProductUOMID, strQuantityOrder, strChanged, strTabId,strMProductID,strCrderID,strManufacturer);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strUOM,
      ...
      String strCrderID,String strManufacturer )
    ...
    if (strChanged.equals("inpmProductUomId") && SLOrderProductData.hasSecondaryUOM(this, strMProductID).equals("1")) {
      ...
      BigDecimal priceActual = new BigDecimal(SLOrderProductData.getOffersPrice(this,dataOrder[0].dateordered,
          dataOrder[0].cBpartnerId,strMProductID,quantityOrder.compareTo(BigDecimal.ZERO)==0?qtyOrdered.toString():quantityOrder.toString(), dataOrder[0].mPricelistId,uomId,strManufacturer, dataOrder[0].id));
      resultado.append("new Array(\"inppriceactual\", " + priceActual.toString()  + "),");
      if (Issotrx.equals("Y")) {
        resultado.append("new Array(\"inppricelist\", " + SLOrderAmtData.getPricelistAmt(this, strMProductID,  plistId, uomId, strManufacturer) + "),");
        resultado.append("new Array(\"inppricestd\", " + SLOrderAmtData.getPricestdAmt(this, strMProductID,  plistId, uomId, strManufacturer,null) + ")");
      } else {
        resultado.append("new Array(\"inppricelist\", " + SLOrderAmtData.getPricestdAmt(this, strMProductID,  plistId, uomId, strManufacturer,dataOrder[0].cBpartnerId) + "),");
        resultado.append("new Array(\"inppricestd\", " + SLOrderAmtData.getPricestdAmt(this, strMProductID,  plistId, uomId, strManufacturer,dataOrder[0].cBpartnerId) + ")");
      }
    }
    resultado.append(");");
