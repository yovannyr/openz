  ...
  public boolean post(String strClave, boolean force, VariablesSecureApp vars,
    ...
    try {
      if (AcctServerData.update(conn, tableName, strClave) != 1) {
        ...
        AcctServerData.insertFactError(econ, conn,AD_Client_ID,AD_Org_ID,"0",DocumentNo,DateAcct,C_DocType_ID,STATUS_DocumentLocked,tableName,strClave);
        econ.commit();
        econ.close();
      ...
      if (getDocumentConfirmation(conn, Record_ID) && post(data, force, vars, conn, con)) {
        ...
        // SZ: Added Accounting Error-Log
        Connection econ=conn.getTransactionConnection();
        AcctServerData.insertFactError(econ, conn,AD_Client_ID,AD_Org_ID,"0",DocumentNo,DateAcct,C_DocType_ID,Status,tableName,strClave);
        econ.commit();
        econ.close();
        errors++;
