...
public class FormDisplayLogic {
     ...
     */
      ...
      while (true) {
        ...
        if (FormDisplayLogicData.isFieldInForm(servlet, fieldid, token).equals("Y") && ! servlet.getCommandtype().equals("NEW")) {
    ...
    private static String getSQL(String sql, ConnectionProvider conn) throws Exception {
        ...
        if(testsql.contains(" as "))
          columname=sql.substring(testsql.indexOf(" as ")+3,testsql.lastIndexOf(" from ")).trim();
        else
          columname=sql.substring(testsql.indexOf("select ")+6 ,testsql.lastIndexOf(" from ")).trim();
