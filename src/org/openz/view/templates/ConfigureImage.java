...
public class ConfigureImage {
  ...
  private static StringBuilder doConfigureAll(HttpSecureAppServlet servlet,VariablesSecureApp vars,Scripthelper script,String fieldname, int leadingemptycols,int colstotal,int maxlength,boolean required,boolean readonly,String callout,String currentvalue,String labeltext, boolean isGrid, String jssettings, String action, String title, String style, String elementId) throws Exception{
    ...
    //stdJSSettings =  "onchange=\"validateTextArea(this.id);logChanges(this);return true;\" oncontextmenu=\"changeToEditingMode('oncontextmenu');\" onpaste=\"changeToEditingMode('onpaste');\" oncut=\"changeToEditingMode('oncut');\" onkeypress=\"changeToEditingMode('onkeypress');\" onkeydown=\"changeToEditingMode('onkeydown');\" onkeyup=\"\"";
     stdhiddenjs ="onchange=\"logChanges(this);return true;\"";
    if (!readonly)
     stdJSSettings =  "onmouseover=\"window.status='Image';return true;\" onmouseout=\"window.status='';return true;\" onclick=\"openServletNewWindow(null, null, '../utility/ImageInfoBLOB?columnName=" + fieldname + "', 'BUTTON', null, true, 600, 900);logChanges(document.getElementById('@FIELDNAME@'));return false;\"";
    for (int i = 0; i < leadingemptycols; i++) {
      retval.append("<td></td>");
    }
    if (! isGrid)
      retval.append(ConfigureLabel.doConfigure(servlet,vars, Sqlc.TransformaNombreColumna(fieldname),"Label_ContentCell",labeltext, elementId));
    Object template =  servlet.getServletContext().getAttribute("imageTEMPLATE");
    if (template==null) {
      template = new String(FileUtils.readFile("Image.xml", directory + "/src-loc/design/org/openz/view/templates/"));
      servlet.getServletContext().setAttribute("imageTEMPLATE", template);
    }
    retval.append(template.toString());
    if (! isGrid)
      Replace.replace(retval, "@CONTENTCELLCLASS@","Image_ContentCell");
      //TextArea_ContentCell
    else
      Replace.replace(retval, "@CONTENTCELLCLASS@","Image_Content");
    if (currentvalue.isEmpty())
      Replace.replace(retval, "@DUMMYCONTENTCELLCLASS@","Image_NotAvailable_medium");
    else
      Replace.replace(retval, "@DUMMYCONTENTCELLCLASS@","");
    ...
    Replace.replace(retval, "@HIDDENJS@",stdhiddenjs);
    Replace.replace(retval, "@NUMCOLS@", Integer.toString(colstotal-1));
    Replace.replace(retval, "@FIELDNAME@",  Sqlc.TransformaNombreColumna(fieldname));
    ...
    Replace.replace(retval, "@STYLE@", "@CURSOR@ style");
    Replace.replace(retval, "@CURSOR@", "cursor: pointer;");
    //retval=Replace.replace(retval, "@ACTION@", action);
