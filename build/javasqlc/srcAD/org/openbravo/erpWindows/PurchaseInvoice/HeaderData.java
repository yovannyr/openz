 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Invoice" +
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , C_Order_ID = (?) , DateOrdered = TO_DATE(?) , DocumentNo = (?) , POReference = (?) , Description = (?) , Internalnote = (?) , C_DocTypeTarget_ID = (?) , DateInvoiced = TO_DATE(?) , DateAcct = TO_DATE(?) , TransactionDate = TO_DATE(?) , SchedTransactionDate = TO_DATE(?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , AD_User_ID = (?) , M_PriceList_ID = (?) , C_Currency_ID = (?) , SalesRep_ID = (?) , IsDiscountPrinted = (?) , C_Charge_ID = (?) , ChargeAmt = TO_NUMBER(?) , PaymentRule = (?) , C_PaymentTerm_ID = (?) , IsGrossInvoice = (?) , A_Asset_ID = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , TotalPaid = TO_NUMBER(?) , DiscountAmt = TO_NUMBER(?) , Writeoffamt = TO_NUMBER(?) , IsPaid = (?) , TotalLines = TO_NUMBER(?) , GrandTotal = TO_NUMBER(?) , DocStatus = (?) , C_DocType_ID = (?) , CopyFrom = (?) , CreateFrom = (?) , DocAction = (?) , Posted = (?) , IsPrinted = (?) , IsTaxIncluded = (?) , DatePrinted = TO_DATE(?) , Processed = (?) , GenerateTo = (?) , UpdatePaymentMonitor = (?) , C_Campaign_ID = (?) , Processing = (?) , IsSelfService = (?) , DueAmt = TO_NUMBER(?) , AD_OrgTrx_ID = (?) , C_Activity_ID = (?) , User1_ID = (?) , User2_ID = (?) , Withholdingamount = TO_NUMBER(?) , DaysTillDue = TO_NUMBER(?) , OutstandingAmt = TO_NUMBER(?) , LastCalculatedOnDate = TO_DATE(?) , PerformancePeriodEnd = TO_DATE(?) , PerformancePeriodStart = TO_DATE(?) , Btnreinvoiceprojectexpenses = (?) , Deliveryrule = (?) , C_Projectphase_ID = (?) , IsPaymentshedulesummary = (?) , Taxdate = TO_DATE(?) , IsSOTrx = (?) , C_Withholding_ID = (?) , IsActive = (?) , C_Invoice_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Invoice.C_Invoice_ID = ? " +
      "        AND C_Invoice.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
