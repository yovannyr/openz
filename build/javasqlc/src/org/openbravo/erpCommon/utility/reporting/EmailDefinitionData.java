...
static Logger log4j = Logger.getLogger(EmailDefinitionData.class);
  ...
  public static String getCentalSenderEmail(ConnectionProvider connectionProvider)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select centralfromemail from c_poc_configuration limit 1          ";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "centralfromemail");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
