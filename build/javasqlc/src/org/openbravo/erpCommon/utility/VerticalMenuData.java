...
static Logger log4j = Logger.getLogger(VerticalMenuData.class);
  ...
  public static String selectOrgName(ConnectionProvider connectionProvider, String userorg)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
    ...
    try {
    st = connectionProvider.getPreparedStatement(strSql);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, userorg);

      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "orgname");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
  ...
  public static String selectOrgImage(ConnectionProvider connectionProvider, String userorg)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "orgimage");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
