...
static Logger log4j = Logger.getLogger(SLOrderDocTypeData.class);
  ...
  public static String selectDocNo(ConnectionProvider connectionProvider, String cDocTypeId, String orgId)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "      SELECT p_documentno::text as p_documentno FROM  ad_sequence_doctype(?,?,'N')";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "p_documentno");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
