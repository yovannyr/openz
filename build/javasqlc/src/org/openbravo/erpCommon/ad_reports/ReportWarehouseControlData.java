...
static Logger log4j = Logger.getLogger(ReportWarehouseControlData.class);
  ...
  public static ReportWarehouseControlData[] select(ConnectionProvider connectionProvider, String adUserClient, String adOrgClient, String parDateFrom, String parDateTo, String reference, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "      FROM (SELECT (M_INOUT.DOCUMENTNO||' '||M_INOUT.MOVEMENTDATE||' '||C_PROJECT.VALUE) AS DESCRIPTION, M_TRANSACTION.M_TRANSACTION_ID, M_INOUT.M_INOUT_ID AS ID, '../'|| (CASE M_INOUT.ISSOTRX WHEN 'Y' THEN 'ShipmentCustomer' ELSE 'MaterialReceipt' END) || '/' || (CASE M_INOUT.ISSOTRX WHEN 'Y' THEN 'Shipment' ELSE 'MaterialReceipt' END) || '_Edition.html' AS UBICATION, 'inpmInoutId' AS INP" +
