...
Select Address
 */
  public static String selectAddress(ConnectionProvider connectionProvider, String C_Location_IDR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT  ( loc.Address1  || ' - ' || loc.Address2  || ' - ' || loc.Postal  || ' - ' || loc.City  || ' - ' || loc.C_Region_ID  || ' - ' || loc.C_Country_ID ) as C_Location_ID " +
      "        FROM C_Location loc " +
      "        WHERE loc.isActive='Y' " +
      "        AND loc.C_Location_ID = ?  ";
