...
*/
  private WorktimeaccountBalance7B81AA6FD4BB40E39BB63E66655D3137Data getEditVariables(Connection con, VariablesSecureApp vars, String strPC_Bpartner_ID) throws IOException,ServletException {
    ...
    try {
    data.cBpartnerId = vars.getStringParameter("inpcBpartnerId");     data.cBpartnerIdr = vars.getStringParameter("inpcBpartnerId_R");     data.wmonth = vars.getStringParameter("inpwmonth");    try {   data.wyear = vars.getNumericParameter("inpwyear");  } catch (ServletException paramEx) { ex = paramEx; }    try {   data.newbalance = vars.getNumericParameter("inpnewbalance");  } catch (ServletException paramEx) { ex = paramEx; }     data.description = vars.getStringParameter("inpdescription");     data.adOrgId = vars.getRequestGlobalVariable("inpadOrgId", windowId + "|AD_Org_ID");     data.adClientId = vars.getRequestGlobalVariable("inpadClientId", windowId + "|AD_Client_ID");     data.cEmpworktimeaccountbalanceId = vars.getRequestGlobalVariable("inpcEmpworktimeaccountbalanceId", windowId + "|C_Empworktimeaccountbalance_ID");     data.isactive = vars.getStringParameter("inpisactive", "N"); 
  ...
  private String getParentID(VariablesSecureApp vars, String strC_Empworktimeaccountbalance_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|C_Empworktimeaccountbalance_ID", data[0].getField("cEmpworktimeaccountbalanceId"));
    }
